...
@Component
public class ThongKeTheoNam extends JFrame {
	...
	//	----------------- Chức năng thống kê -------------------
		btnThngK.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				 findAllByNgayLapHoaDon(listHoaDon,
...
//		----------------- Xuất file excel -------------------
		btnXutExcel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					writeExcel(list,"thongkenam.xlsx");
...
//		----------------- Trực quan hóa dữ liệu -------------------
		btnDuLieu.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				 BarChart_Nam chart = new BarChart_Nam(getListHoaDonForStatis());
//				barChart_AWT.pack();
...
//				 barChart_AWT.setVisible( true ); 
	...
	public List<HoaDon> findAllByNgayLapHoaDon(List<HoaDon> listHoaDon,int namLapHoaDon){
		...
		Map<LocalDateTime, DoubleSummaryStatistics> collect = list.stream().collect(Collectors.groupingBy(HoaDon::getThoiGianLHD,
