...
@Component
public class ThongKeDichVuThangGUI extends JFrame {
...
//				barChart_AWT.setVisible( true ); 
	...
	public List<ThongKeDichVuThang> findAllByThangLapHoaDon(List<HoaDon> listHoaDon,int thangLapHoaDon,int namLapHoaDon){
		List<ThongKeDichVuThang> list=new ArrayList<ThongKeDichVuThang>();
		setLabelEmpty();
		listHoaDon.stream()
		...
		for (HoaDon hoaDon : list) {
			hoaDon.getListDichVu().forEach(dichVu->{
				...
				modelDSHoaDon.addRow(new Object[] {
						...
						dichVu.getTenDichVu(),dichVu.getGia()
				});
			});
